...
const test_service = async function () {
    ...
    try {
        ...
        if (app.blockchains.indexOf('avalanche') === -1)
            throw Error("Failed to load avalanche blockchain");
        //verify avax path
        log.info(tag, "paths: ", app.paths);
        let avaxPath = app.paths.filter((e) => e.symbol === "AVAX");
        log.info(tag, "avaxPath: ", avaxPath);
        assert(avaxPath[0]);
        //verify avax pubkey
        log.info(tag, "app.pubkeys: ", app.pubkeys);
        let avaxPubkey = app.pubkeys.filter((e) => e.symbol === "AVAX");
